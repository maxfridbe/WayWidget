...
run_example() {
    ...
    local cmd="$BINARY --svg $svg --width $w --height $h"
    if [ -n "$script" ]; then
...
case $EXAMPLE in
    "all")
        ...
        # Left side
        $BINARY --svg examples/lcars_clock/widget.svg --script examples/lcars_clock/widget.js --width 600 --height 300 --position 50,50 &
        PID1=$!
        $BINARY --svg examples/ip_visualizer/widget.svg --script examples/ip_visualizer/widget.js --width 350 --height 200 --position 50,400 &
        PID2=$!
        
        # Middle
        $BINARY --svg examples/sunrise/widget.svg --script examples/sunrise/widget.js --width 800 --height 450 --position 700,50 &
        PID3=$!
        $BINARY --svg examples/clock/clock.svg --script examples/clock/widget.js --width 200 --height 200 --position 700,550 &
        PID4=$!

        # Right side
        $BINARY --svg examples/keyboard/widget.svg --script examples/keyboard/widget.js --width 820 --height 350 --position 1550,50 &
        PID5=$!
    ...
    "lion")
        ...
        ;;
    *)
        echo "Usage: ./run.sh [all|clock|lcars|sunrise|keyboard|warpcore|ip|lion]"
        ;;
esac
